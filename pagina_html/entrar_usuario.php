...
<section class="container mt-4">
    ...
    <i id="bookmark-icon-<?= $idInmueble ?>" class="bi bi-bookmark"></i>
</button> </h5>
                                <p class="card-text">
    ...
    <!-- este sript sirve para que el boton de guardado funcione  -->
    <script>
   document.addEventListener('DOMContentLoaded', function() {
    <?php
    // Aquí, dentro de un bucle PHP, puedes generar el código JavaScript necesario para cada botón
    ...
    if ($resultadoInmuebles->num_rows > 0) {
        while ($reg = $resultadoInmuebles->fetch_assoc()) {
            $idInmueble = $reg['IdInmueble'];
            ?>
            const bookmarkButton<?= $idInmueble ?> = document.getElementById("bookmark-button-<?= $idInmueble ?>");

            if (bookmarkButton<?= $idInmueble ?>) {
                bookmarkButton<?= $idInmueble ?>.addEventListener("click", () => {
                    bookmarkButton<?= $idInmueble ?>.classList.toggle("btn-yellow");
                });
            } else {
                console.error('El botón de marcador <?= $idInmueble ?> no fue encontrado.');
            }
        <?php
        }
    }
